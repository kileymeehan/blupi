...
export function SheetsConnectionDialog({
  ...
  // Use board 22 sheets if we're in board 22, otherwise start with empty list
  const [boardSheets, setBoardSheets] = useState<BoardSheet[]>(
    boardId === 22 ? BOARD_22_SHEETS : []
  );
  
  const [activeTab, setActiveTab] = useState<"existing" | "new">(
    boardSheets.length > 0 ? "existing" : "new"
  );
  ...
  // Load sheets on mount (fallback to test data for board 22)
  useEffect(() => {
    const loadSheets = async () => {
      try {
        // When in board 22, we use our pre-defined sheets
        if (boardId === 22) {
          console.log("Using pre-defined sheets for board 22");
          setBoardSheets(BOARD_22_SHEETS);
          setActiveTab(BOARD_22_SHEETS.length > 0 ? "existing" : "new");
          
          // Set initial selected sheet
          if (BOARD_22_SHEETS.length > 0 && !selectedSheetId) {
            setSelectedSheetId(getInitialSelectedId());
          }
        } else {
          // For other boards, fetch from API
