...
interface SimpleFixMetricsProps {
  blockId: string;
  boardId: number;
  className?: string;
...
/**
 * A much simpler metrics component that doesn't rely on complex rendering chains
 * or asynchronous operations that can cause the white screen issue
 */
export const SimpleFixMetrics = forwardRef<SimpleMetricsHandle, SimpleFixMetricsProps>(({
  blockId,
  boardId,
  className = '',
  initialConnection,
  ...
  const savedConnection = useRef(false);
  ...
  useImperativeHandle(ref, () => ({
    openConnectDialog: () => {
      console.log(`Opening dialog for block ${blockId}`);
      setDialogOpen(true);
    }
  }));
  
  // Handle sheet change in dialog
  ...
  const handleSave = () => {
    ...
    try {
      ...
      // Update local state first (this prevents the blank screen)
  ...
  // If no connection data is available, show the unconnected state
      ...
      <>
        ...
        {dialogOpen && (
          <Dialog open={dialogOpen} onOpenChange={(open) => {
            if (!open && !savedConnection.current) {
              setDialogOpen(false);
            }
            // Reset the flag when dialog closes
            if (!open) {
              savedConnection.current = false;
            }
          }}>
